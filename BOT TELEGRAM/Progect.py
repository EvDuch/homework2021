...
from pyowm import OWM #МОДУЛ ДЛЯ РАБОТЫ С ПОГОДОЙ
...
owm = OWM('7f0fb6e211ca80b8042a2197c3030206') #работа с погодой (ВВОДИМ APY-KEY)
...
def send_welcome(message):              #функция принимает сообщение Боту как название города и выполняет работу с температурой нашего города
    place = message.text                #присваиваем переменной название города
    
    #НАХОДИМ ТЕКУЩУЮ ПОГОДУ В НАШЕМ ГОРОДЕ:
    observation = mgr.weather_at_place(place)
    w = observation.weather
    
    #С ПОМОЩЬЮ МОДУЛЯ TRANSLATOR ПЕРЕВОДИМ ПОЛУЧЕННУЮ ИНФОРМАЦИЮ (Т К ИЗНАЧАЛЬНО ОНА ДАНА НА АНГЛИЙСКОМ)
